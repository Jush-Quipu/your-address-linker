...
console.log('\n🔄 Synchronizing versions across all files...');
try {
  execSync('node ' + path.join(__dirname, 'sync-versions.js'), { stdio: 'inherit' });
} catch (err) {
  console.error('❌ Failed to synchronize versions:', err.message);
  process.exit(1);
}
